...
describe('aGroupForRow', () => {
    ...
    it("first true when index === 0", () => {
        let imp = { test: "111" }, index = 0, count = 10, instance = new AGroupForRow(imp, index, count);

        expect(instance.first).toEqual(true);
    })

    it("first false when index !== 0", () => {
        let imp = { test: "111" }, index = 4, count = 10, instance = new AGroupForRow(imp, index, count);
    ...
    it("last true when index === count - 1", () => {
        ...
        instance = new AGroupForRow(imp, index, count)
    ...
    it("even === false when index is not even", () => {
        let imp = { test: "111" }, index = 5, count = 10, instance = new AGroupForRow(imp, index, count);
